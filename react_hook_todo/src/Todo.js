...
export const Todo = () => {
  ...
  //Function to add the todo in the todo list
  const handleSubmit = () => {
    setButton("ADD");
    ...
    if (editId) {
      ...
      const updatedTodos = todos.map((t) =>
        t.id === editTodo.id
          ? (t = { id: t.id, todo: t.data })
          : { id: t.id, todo: t.data }
      ); //if the eklement is desired one then update it else give it default value
  ...
  return (
    <div className="container">
      ...
      <h3>Add Item</h3>
      <p>
        <input id="new-task" type="text" onChange={takeInput} value={todo} />
        ...
        {todos.map((e) => (
          <li key={e.id}>
            <input
              type="checkbox"
              checked={false}
              onClick={() => checkHandle1(e.id)}
            />
            <label>{e.data}</label>
            <button
              className="edit"
              disabled={editButtonStatus}
              onClick={() => handleEdit1(e.id)}
            >
              Edit
            </button>
            <button className="delete" onClick={() => handleDelete1(e.id)}>
              Delete
            </button>
          </li>
        ))}
        ...
        {complete.map((e) => (
          <li key={e.id}>
            ...
            <button
              className="edit"
              disabled={editButtonStatus}
              onClick={() => handleEdit2(e.id)}
            >
              Edit
            </button>
            <button className="delete" onClick={() => handleDelete2(e.id)}>
              Delete
            </button>
          </li>
        ))}
